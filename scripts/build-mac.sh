...
xcrun swiftc -swift-version 5 -O -target arm64-apple-macos14.2 -import-objc-header "$ROOT/mac/AudioRing.h" "$ROOT/mac/Protocol.swift" "$ROOT/mac/AACEncoder.swift" "$ROOT/mac/FLACEncoder.swift" "$ROOT/mac/Capture.swift" "$ROOT/mac/LatencySettings.swift" "$ROOT/mac/DebugSettings.swift" "$ROOT/mac/DebugStats.swift" "$ROOT/mac/DebugWindow.swift" "$ROOT/mac/main.swift" "$BUILD/AudioRing.o" -framework AppKit -framework CoreAudio -framework AVFoundation -framework Network -framework CryptoKit -o "$APP/Contents/MacOS/Unisono"
